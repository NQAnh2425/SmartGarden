...
\* ----------------------------------------------------------------------------- */
uint8_t LoRa_transmit(LoRa* _LoRa, uint8_t* data, uint8_t length, uint16_t timeout,uint8_t *regread,uint8_t *mode_r,uint8_t *mode_r1){
	uint8_t read;

	int mode = _LoRa->current_mode;
	*mode_r1=LoRa_read(_LoRa, RegOpMode);
//	 check is LoRa mode?
	if((*mode_r & 0x80)==0){
		LoRa_write(_LoRa, RegOpMode, 0x88);
		HAL_Delay(10);
	...
	*mode_r=LoRa_read(_LoRa, RegOpMode);
	while(1){
		read = LoRa_read(_LoRa, RegIrqFlags);
		*regread = LoRa_read(_LoRa, RegIrqFlags);
		if(read==0xFF){
			return 0;
		}
		if((read & 0x08)!=0){
...
uint8_t LoRa_receive(LoRa* _LoRa, uint8_t* data, uint8_t length){
	...
	for(int i=0; i<length; i++)
	 ...
	 /* Prevent FIFO overwrite while processing current packet */
	LoRa_gotoMode(_LoRa, STNBY_MODE);
...
uint16_t LoRa_init(LoRa* _LoRa){
	...
	if(LoRa_isvalid(_LoRa)){
		...
		// set bandwidth, coding rate and expilicit mode:
			...
			data=LoRa_read(_LoRa, RegModemConfig2);
			data|=0x04;
			LoRa_write(_LoRa, RegModemConfig2, data);
#endif
...
#ifdef PAYLOAD_CRC_ON
	uint8_t LoRa_isCRCError(LoRa* _LoRa){
		uint8_t data;
		data= LoRa_read(_LoRa, RegIrqFlags);
		return (data & 0x20)>>5;
	}
#endif
